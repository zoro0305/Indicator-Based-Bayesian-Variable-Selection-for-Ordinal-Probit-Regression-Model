...
```{r}
# 1. 計算主要變數的相關係數矩陣
main_vars = train_set_3_interact_standardized[, 3:14]  # 提取12個主要變數
main_cor_matrix = cor(main_vars)

# 2. 初始化變數
remaining = seq_len(ncol(main_vars))  # 未分組的主要變數索引
group_list = list()  # 保存主要變數的分組結果
threshold = 0.93
...
while (length(remaining) > 0) {
  ...
  for (seed in remaining) {
    # 找出與種子列相關係數 > threshold 的所有列
    candidate_group = remaining[main_cor_matrix[seed, remaining] > threshold]
    
    # 檢查候選群組是否為完全子圖
    if (all(main_cor_matrix[candidate_group, candidate_group] > threshold)) {
      # 優先選擇包含列數最多的群組
  ...
  if (best_group_size > 1) {
    group_list[[length(group_list) + 1]] = best_group + 2
  }
...
for (i in seq_along(group_list)) {
  main_group = group_list[[i]]
  sex_interactions = main_group + 12  # `Sex_*` 的索引偏移量
  age_interactions = main_group + 24  # `Age_*` 的索引偏移量
  
  # 將交互作用變數分組
  interaction_groups[[length(interaction_groups) + 1]] = sex_interactions
  interaction_groups[[length(interaction_groups) + 1]] = age_interactions
}

# 5. 整合所有群組
final_groups = c(group_list, interaction_groups)
names(final_groups) = NULL  # 如果不需要名稱
final_groups
```
```{r}
bayes_3_interact_grouped = function(seed_num){
  bayes_model = bayes_ordinal_fitting(train_set_3_interact_standardized, iter=iter, a=1, b=b, theta=0.5, seed_num=seed_num, save=paste0("./PKD_data/bayes_fitting/3_interact/a_1/b_", b, "/grouped_", seed_num, ".rds"), group_list=final_groups)
  return(bayes_model)
}
```
```{r}
...
import_vector_3_interact_grouped = c("b", "iter", "train_set_3_interact_standardized", "bayes_ordinal_fitting", "final_groups")
cl = makeCluster(5)
...
print(best_train_result_grouped$beta_hat[best_train_result_grouped$gamma_hat == 1])
print(best_train_result_grouped$tau_hat)
```
```{r}
print(table(Actual = train_set_3_interact_standardized[, (p+1)], Predicted = Y_hat))
print(max(ACC_train_vector))
...
print(ACC_test)
```
### Refitting with Age, S-R, S-L, AP-R, AP-L, CA by ordinal probit model
```{r}
